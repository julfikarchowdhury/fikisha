    ...
    <div class="row align-items-center">
        <div class="col-md-6 col-lg-3 col-xl-4 col-sm-12">
            <div class="form-group">
                ...
                <select id="items_package_type_id" name="items[{{ $item_number }}][package_type_id]" class="form-control w-100 select2">
                    <option value="1" @selected(old('package_type_id',$request->package_type_id) == 1)>{{ __('parcel.courier_document') }}</option>
                    <option value="2" @selected(old('package_type_id',$request->package_type_id) == 2)>{{ __('parcel.parcel_type') }}</option>
                </select>
        ...
        <div class="col-md-6  col-lg-3 col-xl-4  col-sm-12 mb-2">
            ...
            <div class="form-group">
                <input id="quantity" type="number" name="items[{{ $item_number }}][quantity]" placeholder="{{ __('parcel.quantity') }}" autocomplete="off"
                    class="form-control" value="{{ old('quantity', $request->quantity) }}" required>
            </div>
            @error('quantity')
        ...
        <div class="col-md-6 col-lg-3 col-xl-4 col-sm-12 mb-2 parcelTypeId {{ old('package_type_id',$request->package_type_id) == 2 ? 'd-block' : 'd-none' }}">
            ...
            <div class="form-group">
                <input id="local_weight" type="number" step="any" name="items[{{ $item_number }}][local_weight]" placeholder="{{ __('parcel.weight') }}" autocomplete="off"
                    class="form-control" value="{{ old('local_weight',$request->local_weight) }}">
            </div>
            @error('local_weight')
                <small class="text-danger mt-2">{{ $message }}</small>
            @enderror
                ...
                <div class="preview-block">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input extra_cost" id="extra_cost{{ $item_number }}"
                            name="items[{{ $item_number }}][extra_cost]" @if($request->extra_cost) checked @endif>
                        <label class="custom-control-label" for="extra_cost{{ $item_number }}">{{ __('parcel.extra_cost') }}</label>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-6 col-lg-3 col-xl-4 col-sm-12 extraCostCheck {{ old('extra_cost',$request->extra_cost) ? 'd-block' : 'd-none' }}">
            <div class="form-group">
                <label for="extra_cost_amount">{{ __('parcel.extra_cost_amount') }}<span class="text-danger">*</span></label>
                <input type="number" name="items[{{ $item_number }}][extra_cost_amount]" id="extra_cost_amount{{ $item_number }}" value="{{ old('extra_cost_amount',$request->extra_cost_amount) }}" class="form-control extra_cost_amount" placeholder="{{ __('parcel.extra_cost_amount') }}">
            </div>
        </div>
        <div class="col-md-6 col-lg-3 col-xl-4 col-sm-12 extraCostCheck {{ old('extra_cost',$request->extra_cost) ? 'd-block' : 'd-none' }}">
            <div class="form-group">
                <label for="extra_cost_description">{{ __('parcel.extra_cost_description') }}</label>
                <input type="text" name="items[{{ $item_number }}][extra_cost_description]" id="extra_cost_description{{ $item_number }}" value="{{ old('extra_cost_description',$request->extra_cost_description) }}" class="form-control extra_cost_description" placeholder="{{ __('parcel.extra_cost_description') }}">
            </div>
        </div>
        <div class="col-md-6 col-lg-3 col-xl-4 col-sm-12">
            <div class="form-group">
                <label for="content_parcel">{{ __('parcel.content_parcel') }}</label>
                <input id="content_parcel" type="text" name="items[{{ $item_number }}][content_parcel]" placeholder="{{ __('parcel.content_parcel') }}" autocomplete="off" class="form-control" value="{{ old('content_parcel',$request->content_parcel) }}">
            </div>
        </div>
        <div class="col-md-6 col-lg-3 col-xl-4 col-sm-12">
            <div class="form-group">
                <label for="parcel_value_item_{{ $item_number }}">Parcel Value Amount</label>
                <input id="parcel_value" type="number" step="0.01" name="items[{{ $item_number }}][parcel_value]" placeholder="0.00" autocomplete="off" class="form-control" value="{{ old('parcel_value',$request->parcel_value) }}">
            </div>
        </div>
    </div>
    <div class="row">
