                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb"> 
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('section.index') }}" class="breadcrumb-link">{{ __('levels.section') }}</a></li>
                            <li class="breadcrumb-item"><a href="" class="breadcrumb-link">{{ @$section_type }}</a></li>
        ...
        <div class="  col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ @$section_type }} {{ __('levels.edit') }}</h4>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{route('section.update',$type)}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        ...
                        @elseif ($type == App\Enums\SectionType::WHY_COURIER)
                            @include('backend.front_web.section.why_courier')
                        @elseif($type == App\Enums\SectionType::OUR_WORK_PROCESS)
                            @include('backend.front_web.section.our_work_process')
                        ...
                        @elseif($type == App\Enums\SectionType::BREADCRUMB)
                            @include('backend.front_web.section.breadcrumb')
                        @elseif($type == App\Enums\SectionType::CONTACT)
                            @include('backend.front_web.section.contact')
                        @endif
                        <div class="row">
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12  text-right">
                                <button type="submit" class="btn btn-space btn-primary">{{ __('levels.update') }}</button>
                                <a href="{{ route('section.index') }}" class="btn btn-space btn-secondary">{{ __('levels.cancel') }}</a>
                            </div>
                        </div>
                    </form>
                </div>
