...
class PushNotificationRepository implements PushNotificationInterface
{
            ...
            try {
                if ($request->role_id == 'all') {
                    ...
                    foreach ($users as  $user) {
                        app(PushNotificationService::class)->sendPushNotification($pushNotification, $user->web_token);
                        app(PushNotificationService::class)->sendPushNotification($pushNotification, $user->device_token);
                    }
                } elseif ($pushNotification->user_id) {
                    $users =  User::where('id', $pushNotification->user_id)->get();
                    ...
                    $users = User::where('user_type', $request->role_id)->get();
                    foreach ($users as  $user) {
                        app(PushNotificationService::class)->sendPushNotification($pushNotification, $user->web_token);
                        app(PushNotificationService::class)->sendPushNotification($pushNotification, $user->device_token);
                    }
    ...
    public function update($id, $request)
    {
        try {
            ...
            if (isset($request->image) && $request->image != null) {
                $pushNotification->image_id = $this->file($request->image, $pushNotification->image_id);
            }
            $pushNotification->save();
            return true;
        } catch (\Exception $e) {
    ...
    public function file($file, $file_id = '')
    {
        try {
            $file_name = '';
            if (!blank($file)) {
