    ...
    <div class="container-fluid  dashboard-content">
        ...
        <div class="row">
            <div class="col-lg-6  col-md-6">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                        <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('REVE SMS') }}</h4>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-lg-6  col-md-6">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                        <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('TWILIO SMS') }}</h4>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-lg-6  col-md-6">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                        <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('NEXMO SMS') }}</h4>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-lg-6  col-md-6">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                        <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('BULK GATE') }}</h4>
                    </div>
                    <div class="card-body">
                        <!-- <h4 class="h4 mb-3">{{ __('BULK GATE') }}</h4> -->
                        @if(hasPermission('sms_settings_update') || hasPermission('sms_settings_create'))
                            <form action="{{route('sms-settings.update',\App\Enums\SmsSetup::BULK_GATE)}}"  method="POST" enctype="multipart/form-data" id="basicform">
                                @method('PUT')
                                @csrf
                                <div class="row">
                                    <input type="hidden" value="{{\App\Enums\SmsSetup::BULK_GATE}}" name="smsMethod">
                                    <div class="col-12">
                                        <div class="form-group">
                                            <label for="application_id">{{ __('levels.application_id') }}</label> <span class="text-danger">*</span>
                                            <input id="application_id" type="text" name="application_id" placeholder="{{ __('levels.application_id') }}" autocomplete="off" class="form-control @error('application_id') is-invalid @enderror" value="{{ old('application_id', smsSettings('application_id')) }}" required>
                                            @error('application_id')
                                            <small class="text-danger mt-2">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <div class="form-group">
                                            <label for="application_token">{{ __('levels.application_token') }}</label> <span class="text-danger">*</span>
                                            <input id="application_token" type="text" name="application_token" placeholder="{{ __('levels.application_token') }}" autocomplete="off" class="form-control @error('application_token') is-invalid @enderror" value="{{ old('application_token', smsSettings('application_token')) }}" required>
                                            @error('application_token')
                                            <small class="text-danger mt-2">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <div class="form-group d-flex">
                                            <label for="switch-id">{{ __('levels.status') }}</label>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input switch-id ml-3" name="bulk_gate_status"  id="switch-id" type="checkbox" role="switch"   @if(old('bulk_gate_status', smsSettings('bulk_gate_status')) == \App\Enums\Status::ACTIVE) checked @else @endif>
                                            </div>
                                        </div>
                                    </div>
                                </div>
