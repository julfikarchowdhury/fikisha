            ...
            <div class="  col-sm-12 col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                        <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('levels.service') }} {{ __('levels.edit') }}</h4>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="delivery_type_id" name="delivery_type_id"  class="form-control @error('delivery_type_id') is-invalid @enderror" required>
                                        ...
                                        <option value="1" @selected($service->delivery_type_id == 1)>Inside City</option>
                                        <option value="3" @selected($service->delivery_type_id == 3)>Outside City</option> 
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="shipping_type_id" name="shipping_type_id"  class="form-control @error('shipping_type_id') is-invalid @enderror" required>
                                        ...
                                        @foreach (App\Models\Backend\ShippingType::where('delivery_type_id',$service->delivery_type_id)->get() as $shippingType)
                                            <option value="{{ $shippingType->id }}" @selected($service->shipping_type_id == $shippingType->id)>{{ @$shippingType->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group  col-md-6">
                                    ...
                                    <select name="status" class="form-control @error('status') is-invalid @enderror">
                                        @foreach (trans('status') as $key => $status)
                                            <option value="{{ $key }}"
                                                {{ old('status', $service->status) == $key ? 'selected' : '' }}>
                                                {{ $status }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <textarea class="form-control  @error('description') is-invalid @enderror" name="description" id="summernote"
                                        rows="12">{{ old('description', @$service->description) }}</textarea>
                                    @error('description')
                                        <small class="text-danger mt-2">{{ $message }}</small>
                                    @enderror
                                </div>
