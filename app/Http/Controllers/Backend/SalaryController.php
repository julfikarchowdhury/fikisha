...
class SalaryController extends Controller
{
    ...
    public function store(StoreRequest $request){
        ...
        if((double) $request->amount > (double) $request->account_balance){
            Toastr::warning(__('salary.not_enough_account_balance'),__('message.warning'));
            return back()->withInput();
        }
        $user = User::find($request->user_id);
        if((double) $request->amount > (double) $user->balance){
            Toastr::warning(__('salary.not_enough_user_balance'),__('message.warning'));
            return back()->withInput();
        }
