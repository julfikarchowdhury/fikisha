...
@section('title')
   {{ __('levels.slider') }} {{ __('levels.add') }}
@endsection
@section('maincontent')
        ...
        <div class="  col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('levels.slider') }} {{ __('levels.add') }}</h4>
                </div>
                <div class="card-body">
                    <!-- <h2 class="pageheader-title">{{ __('levels.slider') }} {{ __('levels.add') }}</h2> -->
                    <form action="{{route('slider.store')}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        @csrf
                        <div class="row">
                            
                            <div class="form-group    col-md-6">
                                <label for="title">{{ __('levels.title') }}</label>
                                <input id="title" type="text" name="title" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_title') }}" class="form-control @error('title') is-invalid @enderror" value="{{old('title')}}" >
                                @error('title')
                            ...
                            <div class="form-group    col-md-6">
                                ...
                                <input id="small_title" type="text" name="small_title" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_small_title') }}" class="form-control @error('small_title') is-invalid @enderror" value="{{old('small_title')}}" >
                                @error('small_title')
                                    <small class="text-danger mt-2">{{ $message }}</small>
                                @enderror
                            </div>
