...
@section('title')
    {{ __('parcel.leave') }} {{ __('levels.create') }}
@endsection
@section('maincontent')
                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('hrm.leave.index') }}" class="breadcrumb-link">{{ __('parcel.leave') }}</a></li>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('parcel.leave') }} {{ __('levels.create') }}</h4>
                </div>
                <div class="card-body">
                    <!-- <h2 class="pageheader-title">{{ __('parcel.leave') }} {{ __('levels.create') }}</h2> -->
                    <form action="{{route('hrm.leave.store')}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        @csrf 
                        <div class="row mt-3"> 
                            <div class="col-lg-6 mt-2">
                                <label for="user_id" class="form-label">{{ __('parcel.user') }} <span class="text-danger">*</span></label>
                               <select class="form-control  select2" name="user_id" id="user_id" data-url="{{ route('hrm.leave.assigned.leave') }}">
                            ...
                            <div class="col-lg-6 mt-2">
                               ...
                               <select class="form-control  select2" name="leave_assign_id" id="leave_assign_id">
                                   ...
                                   @if(old('user_id'))
                                        @php
                                            $user   = App\Models\User::find(old('user_id'));
                                            $assignedLeave = App\Models\Backend\HRM\LeaveAssign::where(['role_id'=>$user->role_id,'status'=>App\Enums\Status::ACTIVE])->whereYear('created_at',Date('Y'))->get(); 
                                        @endphp
                            ...
                            <div class="col-12 col-md-3 "> 
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control select2 @error('status') is-invalid @enderror">
                                        @foreach(trans('LeaveStatus') as $key => $status)
                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mt-3">
                                ...
                                <a href="{{ route('hrm.leave.index') }}" class="btn btn-space btn-secondary">{{ __('levels.cancel') }}</a>
                            </div> 
                        </div>
