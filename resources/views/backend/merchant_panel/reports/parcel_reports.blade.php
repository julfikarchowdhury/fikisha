                            ...
                            <div class="form-group col-12 col-md-4  col-sm-6 marchantStatusReportsSelect">
                                ...
                                <select style="width: 100%" id="parcelStatus"  name="parcel_status" class="form-control @error('parcel_status') is-invalid @enderror">
                                    ...
                                    @foreach (trans('parcelStatusFilter') as $key => $status)
                                        <option value="{{ $key}}" @if($request->parcel_status == $key) selected @endif>{{ $status }}</option>
                                    @endforeach
                                </select>
                                @error('parcel_status')
                                <small class="text-danger mt-2">{{ $message }}</small>
                ...
                <div class="card-body">
                    <div class="table-responsive" style="overflow-x:unset">
                        <table class="table" style="width:100%">
                            <thead>
                                <tr style="background-color: var(--warningcolor) !important;color:white">
                                    ...
                                    <th style=" padding: 5px 10px;">Cash Collection</th>
                                    <th style=" padding: 5px 10px;">Delivery Charge</th>
                                    <th style=" padding: 5px 10px;">Vat</th>
                                    <th style=" padding: 5px 10px;">COD</th>
                                    <th style=" padding: 5px 10px;">Total Charge</th>
                                    <th style=" padding: 5px 10px;">Payable</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if (count($parcels) > 0)
                                    @foreach ($parcels as $key=> $parcel)
                                        <tr>
                                            ...
                                            <td style=" padding: 5px 10px;"  >{{ @$parcel->status_name }}</td>
                                            <td>{{ number_format(@$parcel->cash_collection,2) }}</td>
                                            <td style="background-color: rgb(73 73 73 / 7%); padding: 5px 10px;">{{$parcel->delivery_charge }}</td>
                                            <td style="background-color: rgb(73 73 73 / 7%); padding: 5px 10px;">{{@$parcel->vat_amount}}</td>
                                            <td style="background-color: rgb(73 73 73 / 7%); padding: 5px 10px;">{{@$parcel->cod_amount}}</td>
                                            <td style=" padding: 5px 10px;">({{@$parcel->total_delivery_amount + @$parcel->vat_amount}})</td>
                                            <td style=" padding: 5px 10px;">{{@$parcel->current_payable}}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                        <td style=" padding: 5px 10px; text-align: center;" colspan="10">No records available</td>
                                    </tr>
                                @endif
                                <tr style="background-color: var(--warningcolor) !important;color:white">
                                    <th style="text-transform: uppercase; padding: 5px 10px;" colspan="5">Total</th>
                                    <th style=" padding: 5px 10px;"> {{ number_format($parcels->sum('cash_collection'),2) }} </th>
                                    <th style=" padding: 5px 10px;"> {{ number_format($parcels->sum('delivery_charge'),2) }} </th>
                                    <th style=" padding: 5px 10px;"> {{ number_format($parcels->sum('vat_amount'),2) }} </th>
                                    <th style=" padding: 5px 10px;"> {{ number_format($parcels->sum('cod_amount'),2) }} </th>
                                    <th style=" padding: 5px 10px;"> {{ number_format($parcels->sum('total_delivery_amount') + $parcels->sum('vat_amount'),2) }}  </th>
                                    <th style=" padding: 5px 10px;"> {{ number_format($parcels->sum('current_payable'),2) }}  </th>
                                </tr>
                            </tbody>
                        </table>
                    </div>
