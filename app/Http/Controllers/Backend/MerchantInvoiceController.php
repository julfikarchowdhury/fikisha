...
class MerchantInvoiceController extends Controller
{
    ...
    public function InvoicePdf($merchant_id, $invoice_id)
    {
        ...
        if ($this->repo->InvoicePdf($merchant_id, $invoice_id) &&  $this->repo->InvoicePdf($merchant_id, $invoice_id)->invoiceParcels->count() > 0) :
            $invoice = $this->repo->InvoicePdf($merchant_id, $invoice_id);   
            $pdf     = Pdf::loadView('backend.merchant.invoice.invoice_pdf', ['invoice' => $invoice])->setPaper('A4', 'landscape');
    ...
    public function InvoiceCSV($merchant_id, $invoice_id)
    {
        ...
        if ($this->repo->invoiceGet($merchant_id, $invoice_id) &&  $this->repo->invoiceGet($merchant_id, $invoice_id)->invoiceParcels->count() > 0) :
            $invoice = $this->repo->invoiceGet($merchant_id, $invoice_id); 
            return Excel::download(new InvoiceExport($invoice), 'invoice-' . $invoice->merchant->business_name . '-' . $invoice->invoice_date . '.xlsx');
        else:
            Toastr::error(__('parcel.error_msg'), __('message.error'));
            return redirect()->back();
        endif;
