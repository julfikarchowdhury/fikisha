                ...
                <div class="page-breadcrumb">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('departments.index') }}" class="breadcrumb-link">{{ __('town.title') }}</a></li>
        ...
        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('town.create_town') }}</h4>
                </div>
                <div class="card-body">
                    <!-- <h2 class="pageheader-title">{{ __('town.create_town') }}</h2> -->
                    <form action="{{route('towns.store')}}"  method="POST">
                        @csrf
                        
                        <div class="form-group">
                            <label for="city_id">{{ __('town.city') }}</label> <span class="text-danger">*</span>
                            <select name="city_id" class="form-control select2 @error('city_id') is-invalid @enderror" id="city_id">
                                <option value="">{{ __('town.select_city') }}</option>
                        ...
                        <div class="form-group">
                            ...
                            <input id="portal_code" type="text" name="portal_code" placeholder="{{ __('town.Enter_portal_code') }}" autocomplete="off" class="form-control @error('portal_code') is-invalid @enderror" value="{{ old('portal_code') }}" require>
                            @error('portal_code')
                                <small class="text-danger mt-2">{{ $message }}</small>
                            @enderror
                        </div>
