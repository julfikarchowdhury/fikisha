...
function shopDetails() {
    var url = $("select#shopID").data("url");
    shop(url);
}
function shop(url) {
...
toMerchantDetails();
function toMerchantDetails() {
    var url = $("#to_merchant_id").data("url");
    if ($("#to_merchant_id").val()) {
        $.ajax({
            type: "POST",
            url: url,
            data: { id: $("#to_merchant_id").val(), shop: true },
            dataType: "html",
            success: function (data) {
                $("#to_shopID").html(data);
                toShop(url);
...
function deliveryChargeUnitParcel() {
    var total_row = $(".cbm-item");
    total_row.each(function () {
        var merchant_id = $("#merchant_id").val();
        var shipping_type = $("#shipping_type").val();
        var total_cbm = parseFloat($(this).find("#item_total_cbm").val());
        ...
        var weight = parseFloat($(this).find("#item_total_weight").val());
        if (isNaN(weight)) {
            weight = 0;
        }

        var row_length = total_row.length;
        if (
            // merchant_id != "" &&
            shipping_type != "" &&
            length != "" &&
            ...
            var item_unit_service_cost = $(this)
                .find(".item_unit_parcel_service_cost")
                .data("service-cost");
            $.ajax({
                type: "POST",
                url: deliverChargeUnitParcelUrl,
                data: {
                    merchant_id: merchant_id,
                    weight: weight,
                ...
                success: function (data) {
                    $(
                        "#item_unit_parcel_service_cost" +
                            item_unit_service_cost
                    ).val(data.toFixed(2));
                },
            });
        }
...
function getFromHubInfo() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#autocomplete-input").val(data.location);
            // Call Map
            googleMapDirections();
...
$(".from_point_button button").click(function () {
    ...
    if (type == 1) {
        ...
        $("#autocomplete-input").attr("placeholder", "Enter Drop off point");
        getFromHubInfo();
