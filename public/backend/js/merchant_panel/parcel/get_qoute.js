...
function merchantDetails() {
    var url = $("#merchant_id").data("url");
    $.ajax({
        type: "POST",
        url: url,
        data: { id: $("#merchant_id").val(), shop: true },
        success: function (data) {
            $("#shopID").html(data);
            shop(url);
            deliveryCharge();
...
$(document).on(
    ...
    function () {
        ...
        var province_id = $("#to_state_id").val();
        if (account_type && merchant_id && province_id) {
...
var toShopUrl = $("#to_shopID").data("url");
toShop(toShopUrl);
    ...
    if ($("#merchant_id").val()){
        $.ajax({
            ...
            success: function (data) {
                ...
                if (shop.merchant.user.location) {
                    setTimeout(() => {
                        $("#autocomplete-inputs").val(shop.merchant.user.location);
...
function totalSum() {
    ...
    var package_total_row = parseFloat($("#package_total_row").val());
    var payment_mode = $("#payment_mode").val();
    var who_pays_either = $("#who_pays_either").val();
    var sender_payment_mode = $("#sender_payment_mode").val();
    var totalParcelValue = parseFloat($("#total_parcel_value").val());
    var totalAfterDelivery = 0;
    if (package_total_row > 0) {
        if (payment_mode == 2 && who_pays_either == 1 && sender_payment_mode == 1) {
            // Step 1
            totalCurrentAmount = totalParcelValue;
            totalAfterDelivery = totalParcelValue;
        } else if (payment_mode == 2 && who_pays_either == 1 && sender_payment_mode == 3) {
            // Step 2
            totalCurrentAmount = totalParcelValue;
            totalAfterDelivery = (totalParcelValue - totalShippingFee);
        } else if (payment_mode == 2 && who_pays_either == 2 && sender_payment_mode == 2) {
            // Step 3
            totalCurrentAmount += totalParcelValue;
            totalAfterDelivery = totalParcelValue;
        } else if (payment_mode == 2 && who_pays_either == 2 && sender_payment_mode == 1) {
            // Step 4
            totalCurrentAmount = totalParcelValue;
            totalAfterDelivery = totalParcelValue;
        } else if (payment_mode == 1 && who_pays_either == 1 && sender_payment_mode == 1) {
            // Step 5
            totalCurrentAmount = 0;
            totalAfterDelivery = 0;
        } else if (payment_mode == 1 && who_pays_either == 1 && sender_payment_mode == 3) {
            // Step 6
            totalCurrentAmount = 0;
            totalAfterDelivery = -totalShippingFee;
        } else if (payment_mode == 1 && who_pays_either == 2 && sender_payment_mode == 2) {
            // Step 7
            totalCurrentAmount = totalShippingFee;
            totalAfterDelivery = 0;
        } else if (payment_mode == 1 && who_pays_either == 2 && sender_payment_mode == 1) {
            // Step 8
            totalCurrentAmount = 0;
            totalAfterDelivery = 0;
        } else {
        ...
        if (who_pays_either == 1 && sender_payment_mode == 1) {
            // Step 1
            totalCurrentAmount = 0;
            totalAfterDelivery = 0;
        } else if (who_pays_either == 1 && sender_payment_mode == 3) {
            // Step 2
            totalCurrentAmount = totalShippingFee;
            totalAfterDelivery = -totalShippingFee;
        } else if (who_pays_either == 2 && sender_payment_mode == 2) {
            // Step 3
            totalCurrentAmount = totalShippingFee;
            totalAfterDelivery = 0;
        } else if (who_pays_either == 2 && sender_payment_mode == 1) {
            // Step 4
            totalCurrentAmount = 0;
            totalAfterDelivery = 0;
        } else if (who_pays_either == 1 && sender_payment_mode == 1) {
            // Step 5
            totalCurrentAmount = 0;
            totalAfterDelivery = 0;
        } else if (who_pays_either == 1 && sender_payment_mode == 3) {
...
function merchant() {
    ...
    if (
        ...
        merchantCodCharge = merchantData?.cod_charges?.inside_city??0;
        codAmount = parseFloat(
            percentage(cash_collection, merchantData?.cod_charges?.inside_city?? 0)
        );
    } else if (delivery_type_id !== "" && delivery_type_id === "3") {
        merchantCodCharge = merchantData?.cod_charges?.sub_city?? 0;
        codAmount = parseFloat(
            percentage(cash_collection, merchantData?.cod_charges?.sub_city?? 0)
        );
    } else if (delivery_type_id !== "" && delivery_type_id === "4") {
        merchantCodCharge = merchantData?.cod_charges?.outside_city??0;
        codAmount = parseFloat(
            percentage(cash_collection, merchantData?.cod_charges?.outside_city?? 0)
        );
    } else {
        merchantCodCharge = 0;
...
$(document).on(
    "change",
    "#package_type_id,#payment_mode,#who_pays_either, #sender_payment_mode",
    function () {
        totalCBM();
        deliveryCharge();
        totalSum();
...
$("#from_state_id").on("change", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#from_city_id").html(data.city_option);
        },
    });
});
...
$("#to_state_id").on("change", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#to_city_id").html(data.city_option);
        },
    });
});
...
$("#to_state_id, #to_account_type").change(function () {
    ...
    var merchant_id = $("#merchant_id").val();
    var to_account_type = $("#to_account_type").val();
    if (merchant_id && province_id && to_account_type) {
        $.ajax({
            type: "POST",
            url: url,
            data: {
