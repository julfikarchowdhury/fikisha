...
@section('title')
{{ __('country.title') }} {{ __('levels.list') }}
@endsection
@section('maincontent')
<!-- wrapper  -->
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('country.title') }}</h4>
                    @if(hasPermission('country_create') == true )
                    <div class="btn-group">
                        ...
                        <table class="table" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    @if(hasPermission('country_update') == true || hasPermission('country_delete')
                                    == true)
                                    ...
                                    <td>
                                        <div class="row">
                                            ...
                                            <div class="dropdown-menu">
                                                @if(hasPermission('country_update') == true )
                                                <a href="{{route('countries.edit',$country->id)}}" class="dropdown-item">
                                                    <i class="fas fa-edit" aria-hidden="true"></i> {{ __('levels.edit') }}
                                                </a>
                                                @endif
                                                @if(hasPermission('department_delete') == true)
                                                <form id="delete" value="Test" action="{{route('countries.destroy',$country->id)}}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
