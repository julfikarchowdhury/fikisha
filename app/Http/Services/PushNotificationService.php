...
class PushNotificationService
{
    ...
    public function sendStatusPushNotification($parcel, $device_token, $msg, $type = null)
    {
        // if (!empty($topicName)) {
    ...
    public function sendStatusPushNotificationTo($parcel, $token, $msg, $type)
    {
        ...
        $final = [
            'message' => [
                'token' => $token,
                'notification' => [
                    'title' => "Your order #" . $parcel->tracking_id . " status updated " . trans("parcelStatus." . $parcel->status),
                    'body' => $msg,
                ],
                'apns' => [
                    'payload' => [
    ...
    public function sendCreatePushNotificationToMessage($data, $device_token, $type='')
    {
        $accessToken = $this->fcmAccessToken();
        // FCM API URL
        $url = 'https://fcm.googleapis.com/v1/projects/' . env('FCM_PROJECT_ID') . '/messages:send';  // Replace YOUR_PROJECT_ID
    ...
    public function sendCustomPushNotification($token, $title, $msg)
    {
