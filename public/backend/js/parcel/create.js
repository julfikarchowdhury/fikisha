...
function loadCitiesForState(stateId, targetSelectId, onDone) {
    var url = $("#from_state_id").data("url");
    if (!url || !stateId) {
        return;
    }
    $.ajax({
        ...
        success: function (data) {
            var options = populateCityOptions(data);
            $(targetSelectId).html(options).trigger("change");
...
$("#from_state_id").on("change", function () {
    ...
    loadCitiesForState(id, "#from_city_id");
});

$("#to_state_id").on("change", function () {
    var id = $(this).val();
    $("#to_portal_code").val("");
    loadCitiesForState(id, "#to_city_id");
});
...
$(document).on("change", "#customer_id", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#pickup_phone").val(data.mobile || data.phone_number || "");
            $("#sender_email").val(data.email || "");
            $("#pickup_address").val(data.address || "");

            if (data.province_id) {
                $("#from_state_id").val(data.province_id).trigger("change");
                loadCitiesForState(data.province_id, "#from_city_id", function () {
                    if (data.city_id) {
                        $("#from_city_id").val(data.city_id).trigger("change");
                    }
                });
            }
            if (data.portal_code) {
...
$(document).on(
    "keyup change",
    "#cash_collection, #parcel_value, #payment_mode,#who_pays_either",
    function () {
        var cash_collection = parseFloat($("#cash_collection").val());
        if (cash_collection == "" || isNaN(cash_collection)) {
...
function deliveryCharge() {
    var merchant_id = $("select#merchant_id option").filter(":selected").val() || null;
    var shipping_type = "";
...
$(document).ready(function () {
    ...
    $(document).on(
        "keyup",
        ".cbm-item input,#shipping_type,#cash_collection,#special_discount",
        function () {
            totalCBM();
            deliveryCharge();
            totalSum();
    ...
    $(document).on(
        "change",
        "#items_package_type_id,#payment_mode,#who_pays_either, #sender_payment_mode",
        function () {
            totalCBM();
            deliveryCharge();
            totalSum();
    ...
    $(document).on("change", "#shipping_type,#location", function () {
        senderInfo();
                    ...
                    function (results, status) {
                        if (status == google.maps.GeocoderStatus.OK) {
                            if (results[1]) {
                                ...
                                fromGeoHubs(results[0].formatted_address);
                            }
                        }
                    }
                );
    ...
    function getPositionData() {
        ...
        const mapOne = new google.maps.Map(
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data) {
                        ...
                        if (
                            $("#from_state_id").val() == $("#to_state_id").val()
                        ) {
