...
@section('title')
    {{ __('district.title') }} {{ __('levels.edit') }}
@endsection
@section('maincontent')
        ...
        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0" style="font-size: 18px; font-weight: 500;">{{ __('district.edit_district') }}</h4>
                </div>
                <div class="card-body">
                    <!-- <h2 class="pageheader-title">{{ __('district.edit_district') }}</h2> -->
                    <form action="{{ route('districts.update',$district->id) }}"  method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input id="name" type="text" name="name" placeholder="{{ __('placeholder.Enter_name') }}" autocomplete="off" class="form-control @error('name') is-invalid @enderror" value="{{old('name',$district->name)}}" require>
                            @error('name')
                                <small class="text-danger mt-2">{{ $message }}</small>
                            @enderror
                        </div>
